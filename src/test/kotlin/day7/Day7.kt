...
class Day7 {
  ...
  fun sampleSilver() {
    ...
    val solution = listDirectorySizes(database).filter { it < 100000 }.sumOf { it }
  ...
  fun sampleGold() {
    ...
    val folderToDelete = listDirectorySizes(database).filter { it >= neededSpace }.minOf { it }
...
private fun listDirectorySizes(database: MutableList<Descriptor>): List<Int> {
  return database
      ...
      .sorted()
      .distinct()
      .map { database.filter { entry -> entry.directory.startsWith(it) }.sumOf { it.size } }
}
...
fun read_puzzle_input(filename: String): List<String> {
  return File("src/test/kotlin/day7/$filename").readLines()
}
